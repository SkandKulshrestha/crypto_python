...
class SEED(FeistelCipher):
    ...
    def _key_schedule(self):
        ...
        self._round_key = np.zeros((self._no_of_rounds, 2), dtype=np.uint32)
    ...
    def _split_lr(self, buffer: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        half = len(buffer) >> 1
        left = np.zeros((len(buffer),), dtype=buffer.dtype)
    ...
    @staticmethod
    def _convert_to_state(buffer: np.ndarray, state: np.ndarray = None):
        value0 = np.uint32((buffer[0] << 24) ^ (buffer[1] << 16) ^ (buffer[2] << 8) ^ buffer[3])
        value1 = np.uint32((buffer[4] << 24) ^ (buffer[5] << 16) ^ (buffer[6] << 8) ^ buffer[7])
        value2 = np.uint32((buffer[8] << 24) ^ (buffer[9] << 16) ^ (buffer[10] << 8) ^ buffer[11])
        value3 = np.uint32((buffer[12] << 24) ^ (buffer[13] << 16) ^ (buffer[14] << 8) ^ buffer[15])
