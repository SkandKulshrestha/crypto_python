...
from sect_curves import sect_curves
from utility import Utility
from warning_crypto import PointAtInfinity


class SectCurve(ECCCurve):
    def __init__(self, curve_name: str):
        super(SectCurve, self).__init__(curve_name)

    def _initialize_domain_parameters(self):
        if self.curve_name in sect_curves:
            self.m, self.f, self.a, self.b, self.g_x, self.g_y, self.n, self.h, self.S = sect_curves[self.curve_name]
        else:
...
if __name__ == '__main__':
    ...
    curve = SectCurve(curve_name='sect283r1')
